...
PREFILL_LABEL=${PREFILL_LABEL:-qwen4b}
PREFILL_OWNER=${PREFILL_OWNER:-edwardcapriolo}
PREFILL_MODEL=${PREFILL_MODEL:-Qwen3-4B-JQ4}
PREFILL_MODEL_CONFIG=${PREFILL_MODEL_CONFIG:-"$SCRIPT_DIR/benchmarks/configs/qwen3-4b-jq4.json"}
PREFILL_POOL_SIZE=${PREFILL_POOL_SIZE:-16}
PREFILL_TOKEN_COUNTS=${PREFILL_TOKEN_COUNTS:-100,500,1000,10000}
PREFILL_PROFILE_ARGS=${PREFILL_PROFILE_ARGS:-"--profile-stages --profile-rows 40"}
PREFILL_OUTPUT=${PREFILL_OUTPUT:-"$BENCHMARK_RUN_DIR/prefill-token-deep.csv"}
...
printf '\n[prefill-deep] results=%s\n' "$PREFILL_OUTPUT"
printf '[prefill-deep] run_dir=%s\n' "$BENCHMARK_RUN_DIR"
