...
mvn -q -pl core \
  ...
  -Dexec.args="-Djava.library.path=$NATIVE_LIB_DIR --add-modules jdk.incubator.vector,jdk.httpserver,java.net.http --add-opens java.base/java.nio=ALL-UNNAMED --enable-native-access=ALL-UNNAMED -cp %classpath io.teknek.deliverance.benchmark.InferenceBenchmark --engine deliverance --owner tjake --model gemma-2-2b-it-JQ4 ${DELIVERANCE_BENCHMARK_ARGS:---tensor-parallel-size 4 --tensor-parallel-max-ranks-per-worker 2 --tensor-parallel-collective-transport netty --output-head-quantization Q4 --pool-size 16 --max-tokens 256 --warmup-cases 0 --profile-stages --output target/deliverance-tp-netty-benchmark.csv --jsonl-output target/deliverance-tp-netty-benchmark.jsonl}" \
